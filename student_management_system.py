class MyStudent:
    ...
    def set_phone(self, new_phone):
        if new_phone is None or type(new_phone) is not int:
            print("Age must be an integer")
        return
...
def edit_student():
    ...
    Button(edit_window, text="Save", command=lambda: save_and_close(current_student, str_current_name.get(), str_current_age.get(),str_current_phone.get(), str_current_colour.get(), edit_window, str_error_msg)).grid(row=5, column=1, sticky=W)
...
def save_and_close(student, new_name, new_age, new_phone, new_colour, window, error):
    ...
    try:
        ...
        return  # ends the function here, otherwise it will continue and close the window
...
fave_colour = Label(root, textvariable=str_colour).grid(row=3, column=1, sticky=W)

edit_details = Button(root, text="Edit student details", command=lambda: edit_student()).grid(row=4, column=1,
                                                                                              sticky=E + W)
